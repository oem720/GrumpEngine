...
namespace GrumpEngine.Serializer
{
    ...
    {
        public string Serialize(object obj) //TODO: CHANGE THIS TO SAVE TYPE (transplanted here so i wouldn't lose the code)
        {
            bool encoded = false;
            string final;
            ...
            //Step 4a: Serialize each DescriptorString (called during Step 3)
                ...
                sb.Append($"{obj.GetType().Name}=({props[0].Name}=/\"{props[0].GetValue(obj)}\"/,{props[1].Name}={props[1].GetValue(obj)})");
            ...
            else if (IsCustomType(objType) || objType.GetInterface("IEntity") != null)
            {
                ...
                for (int i = 0; i < props.Length; i++)
                {
                    ...
                    {
                        if (props[i].GetValue(obj) != null && props[i].GetValue(obj).GetType().Equals(typeof(string)))
                        {
                            if (i == props.Length - 1)
                            {
